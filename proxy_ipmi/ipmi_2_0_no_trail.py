...
class IPMI20NoTrail():
    ...
    def __init__(self, **keys):
        if len(keys) == 1:
            self.ipmi_auth_type = IPMI20NoTrail.extract_ipmi_auth_type(keys['data'])
            self.ipmi_payload_encrypted = IPMI20NoTrail.extract_ipmi_payload_encryption(keys['data'])
            self.ipmi_payload_authentication = IPMI20NoTrail.extract_ipmi_payload_authentication(keys['data'])
            self.ipmi_payload_type = IPMI20NoTrail.extract_ipmi_payload_type(keys['data'])
            self.ipmi_session_seq = IPMI20NoTrail.extract_ipmi_session_seq(keys['data'])
            self.ipmi_session_id = IPMI20NoTrail.extract_ipmi_session_id(keys['data'])
